...
// regex to find the block starting from /** ローマ字 → カタカナ変換テーブル ... to the end of applyEnglishKatakana function
const oldBlockRegex = /\/\*\*[\s\S]*?ローマ字 → カタカナ変換テーブル[\s\S]*?async function applyEnglishKatakana[\s\S]*?return result;\r?\n\}/m;

if (oldBlockRegex.test(content)) {
    console.log('Regex matched. Replacing content...');
    content = content.replace(oldBlockRegex, replacement);
    fs.writeFileSync(FILE_PATH, content, 'utf8');
    console.log('Successfully updated converter.js');
} else {
    console.error('Regex did not match. Please check the file content.');
}
